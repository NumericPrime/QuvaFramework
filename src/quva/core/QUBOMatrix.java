...
public class QUBOMatrix extends ExecuteProgram implements Cloneable,Serializable{
  ...
  public Map<String, float[]> registeredWeights=new HashMap<String,float[]>();
   ...
   * @param varname name of the variable
   * @param qubitsN number of qubits
   * @param start value of the highest weight. If the value is negative it will take the abs of the value as highest weight but flip the first one.
   * @see quva.core.QUBOMatrix#register(String,int,float,boolean)
   * @see quva.core.QUBOMatrix#register(String,int[],float[])
   * @see  quva.core.QUBOMatrix#genWeights(float, int)*/
	@quva.core.Registers
  public void register(String varname, int qubitsN,float start) {
    register(varname, Qubits.next(qubitsN),genWeights(start*(start<0?-1:1),qubitsN,start<0));
  }
  ...
  public void register(String varname, int[] qubits, float[] weights) {
    registeredWeights.put(varname, weights);
    registeredVars.put(varname, qubits);
  }
  ...
  public float[] findWeight(String varname) {
    return registeredWeights.get(varname);
  }
  ...
  public void multiply(String targ, String n1, String n2, int[]...carries) {
    ...
    multiply(find(targ), findWeight(targ),find(n1), findWeight(n1), find(n2), findWeight(n2), carries);
  }
  /**Sets the variable associated with targ to the product of the two other variables
   ...
   * @throws VarNotRegisteredException when trying to use a non-existent variable
   * @see quva.core.QUBOMatrix#linearEquations(String[])*/
  public void linearEquation(String equation) {
    ...
    linearEquation(sumPart,vars,fact);
  }
  public void linearEquation(float constant,String[] vars,float[] fact) {
	  List<Float> fList=new ArrayList<Float>();
	  for(float f:fact) fList.add(f);
	  linearEquation(constant,Arrays.asList(vars),fList);
  }
  public void linearEquation(float constant,List<String> vars,List<Float> fact) {
	    ...
	    addQuadratic(constant, wrapArr(factArr), wrapArr(varsArr));
  }
  /**Implements a bunch of linear equations
   *@param equations equations to be implemented
   *@see quva.core.QUBOMatrix#linearEquation(String)*/
  public void linearEquations(String... equations) {
  	for(String equation:equations) linearEquation(equation);
  }
  /**Implements polynomial equation. Same as {@link #polynomialEquation(String,String)} with the second arg set to "".
   * @param equation equation to be implemented*/
  public void polynomialEquation(String equation) {
	  polynomialEquation(equation,"");
  }
  /**Implements a list polynomial equations. Runs {@link #polynomialEquation(String)} on all equations.
   * @param equations equations to be implemented*/
  public void polynomialEquations(String... equations) {
	  for(String equation:equations) polynomialEquation(equation);
  }
  ...
  public void registerMultiplyCarries(String target, String n1, String n2) {
    ...
    registerMultiplyCarries(target, find(n1),findWeight(n1), find(n2),findWeight(n2), Qubits.next(qubitsNumb));
  }
  ...
  public void registerMultiplyCarries(String target, String n1, String n2, int[] carriesL) {
    registerMultiplyCarries(target, find(n1),findWeight(n1), find(n2),findWeight(n2), carriesL);
  }
  /**Registered the carries used in multiplication as own variable.
   * @param target name of the variable
   * @param n1 qubits of the first factor
  ...
  public float[] multiplyCarries(String n1, String n2, int[] carriesL) {
    return multiplyCarries( find(n1),findWeight(n1), find(n2),findWeight(n2), carriesL);
  }
  /**Gives the weights of the carries used in a multiplication
   * @param n1 qubits of the first factor
   * @param weights1 qubits of the first factor
  ...
  /**Does exactly the same as limit however this bypasses issues arising from the Quva-Processing library. Also it only allows passing one array
   * @param count number of qubits allowed to be one
   * @param vars qubits involved
   * @see quva.core.QUBOMatrix#limit(float, int[]...)*/
  public void limitBits(float count, int... vars) {
	    float ar[]=new float[vars.length];
	    for (int i=0; i<ar.length; i++) ar[i]=1;
	    addQuadratic(-count, ar, vars);
	  }
  ...
  public static int[] sample(float[][] matrix,QuvaModifiers modifiers,int... mode) {
	  ...
	  mat.Qubits.unassign(range(0,mat.size));
	  modifiers.apply(mat);
  ...
  public void remove(int a) {
	  //TODO fix variable qubit assignement
   ...
   * @see quva.core.QUBOMatrix#readVar(String)
   * @see quva.core.QUBOMatrix#readVar(int[],String)*/
  public float readVar(int[] values,int[] qubits,float[] we) {
	  float ret=0;
	  for(int i=0;i<we.length;i++) ret+=values[qubits[i]]*we[i];
	  return ret;
  }
  ...
  public float readVar(int[] values,String var) {
	  return readVar(values,find(var),findWeight(var));
  }
  ...
  public float readVar(String var) {
	  return readVar(lastRes,find(var),findWeight(var));
  }
